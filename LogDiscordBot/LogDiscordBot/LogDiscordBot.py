...
class LogBotUsers:
    ...
    # Adds a user
    # TODO: Autoscan server for steamID's and save?
...
class LogBotEssentials:
    ...
    # Returning Log details of a player from a logid
    def LogIDdetails(self, logid, steamid3):
        ...
        with urllib.request.urlopen("http://logs.tf/json/" + str(logid)) as url:
            ...
            if steamid3 in data["players"]:
                character = str(data["players"][steamid3]["class_stats"][0]["type"])
                ...
                i = 0
                # Adds every character played in the log to the return object
                for class_stats in data["players"][steamid3]["class_stats"]:
                    returnobject["character" + str(i)] = str(class_stats["type"])
                    i = i + 1
                return returnobject
            else:
    ...
    def LogPlayerSearch(self, steamid64, limit):
        ...
        try:
            ...
            print("Http error got thrown, oops!")
            print(e.reason)
            # TODO: Add a chat message to warn user of an error
        return data

    # Returns String with performance details
    # TODO: Class-specific performance data (E.G: If medic, put ubers and healing)
    def PerformanceDisplay(self, yourorplayer, statsobject):
        ...
        if statsobject["playerinlog"]:
            ...
            charactersList = ["Classes: "]
            #Sort through the statobject's data
            for i in sorted(statsobject.keys()):
                #Find where character data exists
                if(i.startswith("character")):
                    #If there's more than one item going into the list, time to add commas...
                    if "0" not in i:
                        # Well there's no zero in the key, so let's hope it's a number!
                        charactersList.append(", " + statsobject[i])
                    else:
                        # Looks like this is the first or only item, no comma!
                        charactersList.append(statsobject[i])
            charactersList.append("\n")
            perfCharacters = "".join(charactersList)

            perfKDA = "Kills: " + statsobject["kills"] + "\n" \
            ...
            perfTable = perfHeader + perfCharacters + perfKDA + perfFooter
        # Returning
        return perfTable
    ...
    def findMatch(self, minplayers, numoflogs, format, team, message):
        ...
        for i in sortedlogsbytime:
            ...
            if (sortedlogidsbyamount[i] >= minplayers and sortedlogidsbyamount[i] <= format): # TODO: Crashing here?
                matchid = i
...
class LogBotCommands:
    ...
    def command_help(self):
        ...
        # TODO: Why was modhelp on help removed?
        ...
        # if an teamname has been added
            ...
            for team in self.teamlist:
                ...
                if splitmessage[2] in team:
                    ...
                    messagetosend = LogBotEssentials().findMatch(minplayer, 25, team[teamname][0]["type"],
                                                                 team[teamname][1]["players"], self.message)
    ...
    def command_teams(self):
        messagetosend = ":thinking: **Command not found.**"
        ...
        if self.message.content.lower().startswith('!logs teams'):
            ...
            # TODO: Convert this to a massive elif to catch possible edge cases?
            ...
            if self.message.content.lower().endswith('players') and len(messagesplit) == 4:
                ...
                for team in self.teamlist:
                    if messagesplit[2] in team:
                        ...
                        messagetosend = "**" + messagesplit[2] + " players:**\n"
                        num = 0
            ...
            if self.message.content.lower().startswith('!logs teams add'):
                # vars
                messagetosend = ":thinking: " + self.message.author.name + " you don't have permissions."
                messagesplit = self.message.content.lower().split(' ')
                            ...
                            if messagesplit[3] in team:
                                ...
                                # if its a team
                                    ...
                                    if self.message.mentions[0].id in self.userlist:
                                        ...
                                        LogBotTeams().add_team(messagesplit[3], self.message.mentions[0].id,
                                                               messagesplit[5])
                                        ...
                                        messagetosend = self.message.mentions[
            ...
            if self.message.content.lower().startswith('!logs teams create'):
                ...
                if self.message.author.id in self.moderators:
                    ...
                    if notexisting and len(messagesplit) >= 5:
                        ...
                        try:
                            # Error checker: Throws error if the 5th position is not an int
    ...
    def command_user(self):
        # TODO: New command, add a moderator
    ...
    def command_logs(self):
        ...
        if self.message.content.lower() == '!logs':
            # initilaize vars
            messagetosend = "Sorry " + self.message.author.name + " :confused:  but I don't have your steamID. " \
        ...
        elif not len(self.message.mentions) == 0 and len(splitmessage) >= 3 and self.message.content.lower().endswith(
                'profile'):
            # Vars
